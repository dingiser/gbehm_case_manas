...
! PRIVATE MEMBER FUNCTIONS:
  public :: groundfluxes_glacier
  public :: groundtem_glacier
...
!---------------------Argument------------------------------------------
  ...
  real(r8), INTENT(out) :: &! Output fluxes
...
!----------------------- Dummy argument --------------------------------
  real(r8), INTENT(in) :: &
        ...
        zsno       ! roughness length for snow [m]
  real(r8), INTENT(in) :: &! atmospherical variables and observational height        
...
!=======================================================================
      ...
      call meltf_glacier (lb,nl_ice,deltim, &
...
!-----------------------------------------------------------------------
 ...
 subroutine GLACIER_WATER ( nl_ice,maxsnl,deltim,forc_t,&
...
!-----------------------Argument---------- ------------------------------
  ...
  real(r8), INTENT(inout) :: &
       ...
       wa        , &! an adjustment factor, not same as the concept in soil region, by Hy.Li, 2016-7-31
       scv       , &! snow mass (kg/m2)
       snowdp       ! snow depth (m)
...
!=======================================================================
      ...
      ! ! a very weak assumption by HY.Li, should be corrected rapidly.
      ! ! do j=1,nl_ice
      ! j=1
      !   if (wliq_icesno(j)>20. .and. forc_t>=280.) then
      !     gwat = gwat + min(wliq_icesno(j)/deltim,0.5/deltim)
      !     wa = wa - min(wliq_icesno(j),0.5)
      !     wice_icesno(j) = min(wliq_icesno(j),0.5)+wice_icesno(j)
      !     wliq_icesno(j) = wliq_icesno(j)-min(wliq_icesno(j),0.5)
      !   endif  
      ! ! enddo
!=======================================================================
