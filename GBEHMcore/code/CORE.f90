SUBROUTINE CORE (mask,run_pbsm,nl,nl_soil,maxsnl,nl_lake,lon_points,lat_points,deltim,oro,&
           lons, dlon, dlat, ivt, itypwat, lakedepth, dz_lake, & 
           soil_s_v_alb, soil_d_v_alb, soil_s_n_alb, soil_d_n_alb,  &
           wsat,wrsd,watern,alpha,slope,length,elev,Ds,Dr,Dg,kground,    &
           ...
           chil,         ref,          tran,         forc_rh,       &         
           forc_t ,      forc_q,       forc_psrf,    forc_solarin , &
           forc_frl,     forc_prec,    forc_us ,     forc_vs      , &     
           idate,                                                   &
           z_soisno,     dz_soisno,    t_soisno,     wliq_soisno,   &
           wice_soisno,  sst,          smf_soisno,   smf_ice,       &
           ...
           extkb,        extkd,        zwt,          wa,            &
           t_lake,       lake_icefrac, laisun,       laisha,        &
           ...
           ssf,          rssca) 
!=======================================================================
! Core SUBROUTINE coupling GBHM,CLM and PBSM module.
! Coded by Hongyi Li, 2016-5-17
! --------------------------------------------------
!---Land surface module(Colm) 
!   Original author : Yongjiu Dai, 09/30/1999; 08/30/2002, 03/2014
!   Revised : Hongyi Li, 09/20/2015
!
!---GBHM module
!   Created by Dawen Yang, 1998?
!   Modified by Lei Wang,  2006
!   Modified by Bing Gao,  2015
!   Modified by Hongyi Li, 2016
!
!---PBSM module
!   Original method is from Pomeroy's paper (1993,1999,2000)
!   Coded in Fortran 90 by Hongyi Li, 2015
!   Coupled with Colm and GBHM by Hongyi Li, 2016
!=======================================================================
 ...
 IMPLICIT NONE
  ...
  ! ------------------------ Dummy Argument ------------------------------
  integer,  INTENT(in) :: nl          ! number of totail ground layers from soil surface to aquifer bottom
...
! ----------------------
  real(r8), INTENT(in) :: &
        ...
        porsl(nl,lon_points,lat_points)  , &! fraction of soil that is voids [-]
        psi0(nl,lon_points,lat_points)   , &! minimum soil suction [mm]
        wfld(nl,lon_points,lat_points)   , &! water field capicity
        bsw(nl,lon_points,lat_points)    , &! clapp and hornbereger "b" parameter [-]
        hksati(nl,lon_points,lat_points) , &! hydraulic conductivity at saturation [mm h2o/s]
        csol(nl,lon_points,lat_points)   , &! heat capacity of soil solids [J/(m3 K)]
        dksatu(nl,lon_points,lat_points) , &! thermal conductivity of saturated soil [W/m-K]
        dkdry(nl,lon_points,lat_points)  , &! thermal conductivity for dry soil  [J/(K s m)]
  ...
  real(r8), intent(in) :: & ! added by HONGYILI, 2015.12.13
        ...
        elev   (lon_points,lat_points)    , &   ! elevation (m)
        kground(lon_points,lat_points)          ! GW hydraulic conductivity (m/s)    

  real(r8), intent(in) :: & ! added by HONGYILI, 2016.8.11
        rssca  (lon_points,lat_points) ! remotely sensed snow cover area (1: snow; 0: snow free)
...
! ----------------------
  real(r8), INTENT(in) :: &
      ...
      forc_solarin (lon_points,lat_points), &! solar rad onto srf [W/m2]
      forc_frl     (lon_points,lat_points), &! atmospheric infrared (longwave) radiation [W/m2]
      forc_prec    (lon_points,lat_points), &! precipitation [mm/s]
...
! ----------------------------------------------------------------------
  ...
  real(r8), INTENT(inout) :: &
        ...
        smf_ice    (1:nl_soil,lon_points,lat_points),&! snowmelt fraction in soil ice  (0-1).Added by HY,2016-11-30
...
! additional diagnostic variables for output
  real(r8), INTENT(out) :: &
        ...
        h2osoi(nl,lon_points,lat_points)! volumetric soil water in layers [m3/m3]
...
! ----------------------------------------------------------------------
  ...
  real(r8), intent(out) :: &                !Added for MODEL development
        ...
        qsubl     (lon_points,lat_points), &! total sublimation flux (mm H2O /s)
        qfros     (lon_points,lat_points), &! total snow surface dew flux (mm H2O /s)
        snoweva   (lon_points,lat_points), &! total snowmelt evaporsublimation flux (mm H2O /s)
...
!====================F2PY===========================
!f2py intent(in)      run_pbsm,nl,nl_soil,nl_lake,maxsnl,lon_points,lat_points,mask
...
!f2py intent(in)      wfld,wsat,wrsd,watern,alpha,slope,length,elev,Ds,Dr,Dg,kground  
...
!f2py intent(in)      smpmax,smpmin,trsmx0,tcrit,forc_q,forc_t      
!f2py intent(in)      forc_psrf,forc_solarin,forc_frl,forc_prec,forc_us,forc_vs,forc_rh 
!f2py intent(in,out)  z_soisno,dz_soisno,t_soisno,wliq_soisno,wice_soisno,smf_soisno,smf_ice
!f2py intent(in,out)  t_lake,lake_icefrac,oro,t_grnd ,tlsun,tlsha,ldew,sag,sst,scv     
...
!f2py intent(out)     xerr,zerr,tref,qref,trad,rsur,rnof,qlat,qground,qsmelt,qsubl,qfros,snoweva
...
!f2py depend(lon_points,lat_points) wsat,wrsd,watern,alpha,slope,length,elev,Ds,Dr,Dg,kground,Drw
...
!f2py depend(lon_points,lat_points) forc_t,forc_q,forc_psrf,forc_solarin,forc_frl,forc_rh   
!f2py depend(lon_points,lat_points) forc_prec,forc_us,forc_vs,oro
...
!f2py depend(lon_points,lat_points) albg,albv,alb,ssun,ssha,thermk,qsubl,qfros,snoweva
...
!f2py depend(nl_soil,lon_points,lat_points)     rootfr,smf_ice,smf_soisno
!f2py depend(nl,lon_points,lat_points)     wfld,h2osoi,porsl, psi0,bsw,hksati,csol,dksatu,dkdry
...
!--------------------remotely sensed data----------------
!f2py depend(lon_points,lat_points) rssca
!===================END F2PY========================        
...
! -------------- Local varaibles -------------------
  ...
  ! GBHM needed      
  real(r8) :: &
        runoff      (lon_points,lat_points) , &   ! surface runoff
...
! -----------------------------------------------------------------------------------------
  real(r8) :: debugvar1,debugvar2,debugvar3 ! Tempporal variables for debug     
! -----------------------------------------------------------------------------------------
      ...
      DO j = 1, lat_points
        DO i = 1, lon_points 
         if (mask(i,j)==0) then
            ...
            CALL format_clmforcing( &
                forc_t(i,j),        forc_q(i,j),        forc_psrf(i,j),     forc_solarin(i,j), &
                forc_prec(i,j),     forc_pco2m(i,j),    forc_po2m(i,j),     forc_prc(i,j),     &
                forc_prl(i,j),      forc_pbot(i,j),     forc_sols(i,j),     forc_soll(i,j),    &
                forc_solsd(i,j),    forc_solld(i,j),    forc_hgt_u(i,j),    forc_hgt_t(i,j),   &
                forc_hgt_q(i,j),    forc_rhoair(i,j))  

            CALL CLMMAIN (i,j,deltim,oro(i,j), nl, nl_soil, nl_lake, maxsnl,&
                lons(i), dlon(i,j), dlat(i,j), ivt(i,j), itypwat(i,j), &
                lakedepth(i,j), dz_lake(1:,i,j), &
              ...
              ! LAND SURFACE VARIABLES REQUIRED FOR RESTART
                ...
                sst(i,j),          ssf(i,j),        smf_soisno(maxsnl:,i,j),                &
                smf_ice(1:,i,j),&
              ...
              ! additional diagnostic variables for output
                ...
                Drw(i,j),          qground(i,j),      qsmelt(i,j),       qsubl(i,j),        &
                qfros(i,j),        snoweva(i,j), &
