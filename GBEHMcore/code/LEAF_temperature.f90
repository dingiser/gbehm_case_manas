...
!!!!!ERROR to be corrected
       if(zldis <= 0.0) then
          !write(6,*) 'the obs height of u less than the zero displacement heght',hu, displa
          zldis = 0.1 !added by Hongyi,2015-10-25
          !call abort
       endif
...
! Debug: limit negative fevpl vaules, I dont know it is right or not, need further job, by HY, 2016-7-4
      if (fevpl<-1.) then
        fevpl = -1.
      endif  
      if(etr.ge.sigf*etrc)then
...
! Debug: limit negative etr vaules, I dont know it is right or not, need further job, by HY, 2016-5-4
      if (etr<0.) then
        etr = 0.
      endif        
      ! wind stresses 
...
!-----------------------------------------------------------------------
      ...
      erro = sabv + irab + dirab_dtl*dtl(it-1) - fsenl - hvap*fevpl

! #if(defined CLMDEBUG)
      if(abs(erro) .gt. .2) &
      write(6,*) 'energy imbalance in leaftemone.F90',it-1,erro,sabv,irab,fsenl,hvap*fevpl,us,vs,qm
! #endif
...
! vegetation parameters
  real(r8), INTENT(in) :: &
        ...
        z0m,        &! roughness length, momentum [m]
        effcon,     &! quantum efficiency of RuBP regeneration (mol CO2 / mol quanta)
        vmax25,     &! maximum carboxylation rate at 25 C at canopy top; the range : 30.e-6 <-> 100.e-6 (mol co2 m-2 s-1)
        shti,       &! slope of high temperature inhibition function     (s1)
        hhti,       &! 1/2 point of high temperature inhibition function (s2)
        slti,       &! slope of low temperature inhibition function      (s3)
        hlti,       &! 1/2 point of low temperature inhibition function  (s4)
...
!      rsoil = 5.22 * 1.e-6
       ...
       if (zldis<=0.) zldis=0.1 !corrected by Hongyi ,2015-2-20
...
!if(defined CLMDEBUG) 
       if(abs(erro) .gt. .2) write(6,*) 'leaftemtwo.F90 : energy imbalance',erro, it-1
!#endif
...
! the condition is set to limit the etr vaule, I dont know it is right or not , by HY, 2016-5-4
      if (etr<0.) then
        etr = 0.
      endif 
      if(etr.ge.sigf*etrc)then
