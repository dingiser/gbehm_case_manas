...
! ------------------------------------------------------------------
subroutine read_basin(nrows,ncols,nc,&
    ...
    implicit none
    ! include 'dims2.inc'
    integer,INTENT(in) :: ncols,nrows,nc
    real(r8),INTENT(out) :: &
    ...
    integer,INTENT(out) :: &
                ...
                basinmap(nrows,ncols) ! subbasin NO. distribution
                
    character*6,INTENT(out) ::  subbasin(nc)      ! name of sub-basin 
!f2py intent(in)  nrows,ncols,nc  
!f2py intent(out) subbasin,nsub,nflow,dx,Dr,s0,b,roughness,ngrid,grid_row,grid_col
!f2py intent(out) psubbasin,nbasinup,pbasinup,Dr_grid,basinmap                
    ...
    integer       subno(nc) !number of of sub-basin 
    
    para_dir   ="parameter/"
    call strlen(para_dir,ib1,ib2)
    open(1,file = para_dir(ib1:ib2)//'subbasin.dat', status='old')
    do i = 1,nc
       read(1,*)nsub,psubbasin(i),(pbasinup(i,j),j=1,4)!3?
    enddo
    close(1)
    ...
    do isub = 1, nsub
        ...
        do iflow = 1, nflow(isub)
            ...
            do ig = 1,ngrid(isub,iflow)      
                ...
                basinmap(grid_row(isub,iflow,ig),grid_col(isub,iflow,ig))= subno(isub)
            end do
        end do
        close(3)
    end do
...
! -----------------------------------------------------------------
subroutine hillslope_route_model(nrow,ncol,nc,subbasin,isub,nflow,psubbasin,nbasinup,&
          nsub,ngrid,grid_row,grid_col,start, &
          year,month,day,hour,dayinmonth,startyear,endyear,startmonth,endmonth,startday,endday, dt, &
    ...
    implicit none
    ! include 'dims2.inc'
    integer, INTENT(in) :: nrow,ncol,nc
...
real(r8), INTENT(inout) ::  Drw  (nc,nx)       ! water depth in the river of the flow interval (m)
real(r8), INTENT(inout) ::  sst  (nrow,ncol) 
...
!f2py intent(in) nrow,ncol,nc,subbasin,isub,nflow,psubbasin,nbasinup,nsub,ngrid,grid_row,grid_col
!f2py intent(in) start
!f2py intent(in) year,month,day,hour,dayinmonth
!f2py intent(in) startyear,endyear,startmonth,endmonth,startday,endday, dt
...
!f2py intent(out) q2,Qh
    real(r8)    qlin2(nc,nx)! lateral inflow of current time step (m^3/m/s)
    real(r8)    qin(nx)     ! lateral inflow of one flow interval(m3/sec/m),unit (m3/s/m, flow into river)
    integer     iflow       ! flow-interval number
    real(r8)    q_hillslope ! surface runoff of one simulation 
    real(r8)    qin_tmp
    ...
    real(r8)    waterhead
    real(r8)    power    
    ! real(r8)    Drw(nc,nx)         ! water depth in the river of the flow interval (m)
...
! c----------------------------------------------------------------------
    ...
    real(r8)    tmp, value         ! temporary variables
    integer     i, j, k, m         !, n      ! temporary variables
    integer     tmpj,tmpim,tmpid
    integer     ia1, ia2, ib1, ib2, ic1, ic2
    real(r8)    criterion, h1, h2, f, df
    real(r8)    q1_upper, q2_upper,rs    !, i_sub,  lo_dis
    integer     ii1, ii2           ! , ii3
    real(r8)    Qtotal,basinarea
    real(r8)    b_tmp              ! adjust riverbed-width during heavy-flood
    real(r8)    rs_tmp             ! adjust rs during heavy-flood
    integer     inicon
...
! -------------------------------------
      do 999 iflow=1,nflow(isub)
        ...
        do 666 ig=1,ngrid(isub,iflow)
         ...
         qin_inter = runoff_inter(ir,ic)*length(ir,ic)          !to m/s !change2 
...
! cc     :           + qin_inter + qin_g)* area(ir,ic)/length(ir,ic)        ! m3/s, one flow-interval
...
! ---------------------------------------------------------------------
    ...
    ! Default to output hydro-yearly discharge in each Dec 31.
    ! if endmonth and and day are specified, output the discharge also.
    if((month.eq.endmonth .and. day.eq.endday .and. hour.eq.24) .or. (month.eq.12 .and. day.eq.31 .and. hour.eq.24)) then
        call strlen(simul_dir, ia1,ia2)
...
!**********************************************************************
    ...
    real(r8) Q01, Q02, Q1, Q2
    real(r8) qlin0, qlin
    real(r8) dt
    real(r8) dx, b, s0, roughness
    real(r8) y, p, beta, criterion
