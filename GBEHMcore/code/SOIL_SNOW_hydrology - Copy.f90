...
!                   
  ...
  real(r8) :: evpsm,qinsm
  real(r8) debugtmp1,debugtmp2,debugtmp3
...
! endif
  ...
  if (lb>=1)then
    ...
    if (pg_rain>0.) then
      ! smf_snow = 1. ! assume that liquid water in snow are all from snowmelt, including rain hold in snowpack
      smf_snow = smf_soisno(-1) ! assume that liquid water in snow are all from snowmelt, including rain hold in snowpack
...
!=======================================================================
      ...
      call gbhm_recharge(deltim,nl_soil,zwt,Dg,Ds,wa,GWcs,wsat,wrsd,watern,alpha,wfld,&
                      hksati(1:nl_soil)*0.001,dz_soisno(1:nl_soil),wliq_soisno(1:nl_soil),wice_soisno(1:nl_soil),&
                      qcharge)  
...
! if(debugtmp2-qsmelt*deltim-debugtmp3>1E-8) print*, 'error in subsurfacerunoff',debugtmp2-debugtmp3,qsmelt*deltim                
      ! total runoff (mm/s)
      rnof = rsubst + rsur   
...
!-----------------------------------------------------------------------
    ...
    real(r8), INTENT(inout) :: qcharge           ! aquifer recharge rate (positive to aquifer) (mm/s)
...
!
    ...
    real(r8) :: hksat_inter,qmax,qmin,temq(1:nl_soil)
!-----------------------------------------------------------------------
    ...
    do j = 1, nl_soil
       if(t_soisno(j)>tfrz) then
          if(porsl(j)<1.e-6)then     ! bed rock
             ...
             ! s_node = max(vol_liq(j)/porsl(j),wrsd/porsl(j))!modified by HY, 2016-4-25
...
! Method II: 
           ...
           ! hksat_inter = (zmm(j+1)-zmm(j))/((zimm(j)-zmm(j))/hksati(j)+(zmm(j+1)-zimm(j))/hksati(j+1))
           ! s1 = (vol_liq(j)*(zimm(j)-zmm(j)) + vol_liq(j+1)*(zmm(j+1)-zimm(j))) &
           !    / (porsl(j)*(zimm(j)-zmm(j)) + porsl(j+1)*(zmm(j+1)-zimm(j)))
           ! s1 = min(1.,s1)
           ! s2 = hksat_inter*s1**(2.*bsw(j)+2.)
           ! hk(j) = s1*s2
           ...
           ! dhkdw2(j) = (2.*bsw(j)+3.)*s2*(zmm(j+1)-zimm(j))/(porsl(j)*(zimm(j)-zmm(j))+porsl(j+1)*(zmm(j+1)-zimm(j)))
             ...
             ! call conductivity_V(hksati(j)*0.001,porsl(j),wrsd,watern,vol_liq(j),hk(j)) !modified by HY, 2016-1-26
             ! hk(j) = hk(j)*1000.
             hk(j) = hksati(j) * (vol_liq(j)/porsl(j))**(2.*bsw(j)+3.) !used in origin CLM
             dhkdw1(j) = hksati(j) * (2.*bsw(j)+3.)*(vol_liq(j)/porsl(j))**(2.*bsw(j)+2.)/porsl(j)
             dhkdw2(j) = 0.
          endif
...
!   else
       qout(j) = hk(j)    
       ! qout(j) = qcharge   !added by HY, debug
       dqodw1(j) = dhkdw1(j) !*qcharge /hk(j) ! changed by HY,debug: dqodw1(j) = dhkdw1(j)
       dqodw2(j) = 0.
!   endif
    !dhkdw1 = d(hk)/d(vol_liq(j))
    !dhkdw2 = d(hk)/d(vol_liq(j+1))
    !dqodw1 = d(qout)/d(vol_liq(j))
    !dqodw2 = d(qout)/d(vol_liq(j+1))
    amx(j) = -dqidw0(j)
    bmx(j) =  dzmm(j)/deltim - dqidw1(j) + dqodw1(j)
    cmx(j) =  dqodw2(j)
    rmx(j) =  qin(j) - qout(j) - etr*rootr(j)
...
! The mass balance error (mm) for this time step is
    ...
    if(abs(errorw) > 1.e-3)then
       write(6,*) 'mass balance error in time step =',errorw,etr
    endif
!#endif

    ! Recharge rate qcharge to groundwater (positive to aquifer)
    qcharge = qout(nl_soil) + dqodw1(nl_soil)*dwat(nl_soil) 
if (isnan(dwat(nl_soil)).or.isnan(qcharge)) then
    print*,'debug_qcharge',qcharge,qout(nl_soil), dqodw1(nl_soil),dwat(nl_soil),hk(nl_soil),wliq_soisno(:)
    dwat(nl_soil) = 0.
    qcharge = 0.
endif
      ! update the mass of liquid water (Available if use soilwater module (CLM))
    do j= 1, nl_soil
...
!
    ...
    real(r8), parameter :: watmin = 1.0e-10  ! Limit irreduciable wrapping liquid water 
                                          ! a tunable constant
...
!=======================================================================
    CALL lateral_runoff(hk,anik,nl_soil,jwt,dz_soisno,eff_porosity,deltim,&
                        ...
                        wliq_soisno,qlat,smf_soisno(1:),qsmelt)
...
! ---------------------------------------------------------------    
    ...
    qsmelt = qsmelt + qground*smf_soisno(nl_soil+1)
  ...
  subroutine lateral_runoff(ksoil,anik,nl_soil,isat,deltz,eff_porosity,dt,ss_f,&
                            wfld,wsat,wrsd,watern,alpha,slope,length,wliq_soisno,qlat,smf_soisno,qsmelt)
    ...
    ! if(wfld.lt. 0.60*wsat) wfld = 0.60*wsat
  ! if(wfld.gt. 0.85*wsat) wfld = 0.85*wsat
    ! wfld = 0.1*wsat
    ! tempwfld = 0.7*eff_porosity
    qlat= 0.
    tmp = 0. 
    j = min(isat, nl_soil)
    do i = 1, j
      tmp = tmp + deltz(i)
      ! call SuctionFromMoisture_V(w(i),eff_porosity(i),wrsd,watern,alpha,suction)
      ! call MoistureFromSuction_V(wfld,eff_porosity(i),wrsd,watern,alpha,suction)
      ! wfld = 0.1*eff_porosity(i)
      ! if(wfld.lt. 0.60*eff_porosity(i)) wfld = 0.60*eff_porosity(i)
      ! if(wfld(i).gt. 0.85*eff_porosity(i)) wfld(i) = 0.85*eff_porosity(i)
      ! if((w(i)-wfld) .gt. 0.1E-3) then
      ! wfld = 0.1*eff_porosity(i)
      if((w(i)-wfld(i)) .gt. 0.1E-3) then
...
! ----------------------------------------------------------------------
  ...
  ! ----------------------------------------------------------------------
    ...
    real(r8), intent(out) :: &
               rech
    integer  nuz,i
    real(r8) GWst,tmp,ps1,ps2,tmp2,tmpw,tmpavk,deficit,w_sum0,w_sum1,erro
    real(r8) avz(nl_soil),avk(nl_soil)
    real(r8) w(1:nl_soil) !Soil moisture,% 
    real(r8) wice(1:nl_soil)
    
    ! w_sum0 = sum(wliq_soisno(1:)) + sum(wice_soisno(1:)) + wa
    rech = 0.0
    tmp  = 0.0    
    nuz  = nl_soil
    GWst = wa/1000.
    w(1:nl_soil)   = wliq_soisno(1:nl_soil)*0.001/deltz(1:nl_soil)
    ...
    if(Dgl .ge. Ds) then
       ...
       CALL conductivity_V(k0(nuz),wsat,wrsd,watern,w(nuz),tmpavk)
       avk(nuz) = tmpavk
       ...
       rech = rech*dt
       if(rech .ge. 0.0) then
         tmp = (w(nuz)-wfld(nuz))*deltz(nuz)
         tmp = amax1(tmp, 0.0)
         tmp2= Dg*GWcs - GWst
         if (tmp2 .le. 0.) tmp2 = 0.
         rech= amin1(rech, tmp, tmp2)
         if(rech.lt.-0.1E-6)  then
            print *,"rech<0",rech,tmp,tmp2,GWst, GWcs
         end if
       else
          rech   = amax1(rech, -1.0*GWst)
          tmpw   = wsat-wice(nuz)
          tmp    = amax1( 0.0, (tmpw-w(nuz))*deltz(nuz) )
          rech   = amax1(rech, -1.0*tmp)
          if(rech.gt.1.0E-10) print *,"debug,rech>0" ,rech,tmp,GWst, GWcs
       endif
       if(abs(rech) .lt. 0.1E-20) rech = 0.0
    endif 
