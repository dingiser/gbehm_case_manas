...
! ground cover index
 integer, INTENT(in) :: &
      itypwat     ! land water type (0=soil, 1=urban or built-up, 2=wetland,3=land ice, 4=deep lake, 5=shallow lake)
 real(r8), INTENT(in) :: & 
...
! set default soil and vegetation albedos and solar absorption
      ...
      albg(:,:) = 0.2 ! ground
      albv(:,:) = 0.2 ! vegetation
      albsno(:,:)=0.8         !set initial snow albedo, corrected by HY: = 0.8
      ssun(:,:) = 0.
      ssha(:,:) = 0.
     tranc(:,:) = 0.
      thermk = 1.e-3
      extkb = 1.e-6
      extkd = 0.718
! print*, 'debug albedo scv' , scv   ,coszen    
...
! ----------------------------------------------------------------------
      if(scv>0.)then
         ...
         albsno(1,2) = dfalbs
         albsno(2,2) = dfalbl
! print*, 'debug albedo snow' , albsno       
      endif
...
! parameters
  real(r8), intent(in) :: &! static parameters associated with vegetation type
            ...
            ref(2,2),      &! leaf reflectance (iw=iband, il=life and dead)
            tran(2,2)       ! leaf transmittance (iw=iband, il=life and dead)
  real(r8), intent(in) :: &! time-space varying vegetation parameters
            green,         &! green leaf fraction
