SUBROUTINE IniTimeConst (nl_soil , zsoih , ivt   ,itypwat, &
...
!--------------------------- Output
  integer, INTENT(out) :: itypwat  !3=glacier/ice sheet, 4=deep lake, 5=shallow lake)
...
! #if (defined USGS_CLASSIFICATION)
      ...
      if(i==16)          itypwat=4  ! land water bodies
...
! #if (defined IGBP_CLASSIFICATION)
      ...
      ! if(i==17)          itypwat=4  ! land water bodies
      ! if(i==0)           itypwat=99 ! ocean
! #endif
...
! #if (defined USGS_CLASSIFICATION)
    ...
    if( i > 0 )then             ! land grids
      ...
      ! ----------------------------------------------------------
      if(itypwat>=3)then  !glacier/ice sheet or land water bodies or ocean
         rootfr(:)=0.
      else
